...
abstract class BaseModel
{
    ...
    // Cache dei nomi colonna per tabella (solo driver database)
    ...
    /**
     * Ottiene e cache i nomi delle colonne della tabella (solo database)
     */
    protected static function getTableColumns(): array
    {
        ...
        if (!isset(self::$tableColumnsCache[$table])) {
            // Assumiamo schema 'public' per PostgreSQL
            $rows = DB::select(
    ...
    {
        // Gestione flessibile dei parametri come in lz13: 2/3/4 argomenti
        $numArgs = func_num_args();

        // Se viene passato $relations come 4° parametro, imposta eagerLoad
        ...
        // Se solo 2 parametri, oppure 3 dove $value è null e $operator non è un operatore valido,
        // allora il secondo parametro è il valore e l'operatore diventa '='
    ...
    public function save(): void
    {
        ...
        if (empty($existingColumns) || in_array('created_at', $existingColumns)) {
            $this->created_at = $this->created_at ?: $now; // default se non è settato
        }
        if (empty($existingColumns) || in_array('updated_at', $existingColumns)) {
            $this->updated_at = $now; // aggiorniamo solo se la colonna esiste
        }
        ...
        if (static::$driver === 'json') {
            ...
            // Filtra i bindings per includere solo colonne esistenti nella tabella
